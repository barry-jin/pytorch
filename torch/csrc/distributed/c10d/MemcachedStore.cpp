...
class MemcachedStoreMasterDaemon : public BackgroundThread {
 public:
  explicit MemcachedStoreMasterDaemon(Socket&& storeListenSocket);

  ~MemcachedStoreMasterDaemon() override;
...
MemcachedStoreMasterDaemon::MemcachedStoreMasterDaemon(Socket&& storeListenSocket)
    : BackgroundThread{std::move(storeListenSocket)} {
  daemonThread_ = std::thread{&MemcachedStoreMasterDaemon::run, this};
}

MemcachedStoreMasterDaemon::~MemcachedStoreMasterDaemon() {
  if (memcachedClient_) {
    memcached_free(memcachedClient_);
    memcachedClient_ = nullptr;
  }
  dispose();
}

void MemcachedStoreMasterDaemon::queryFds(std::vector<struct pollfd>& fds) {
...
void MemcachedStoreMasterDaemon::query(int socket) {
  QueryType qt;
...
void MemcachedStoreMasterDaemon::wakeupWaitingClients(const std::string& key) {
...
void MemcachedStoreMasterDaemon::sendKeyUpdatesToClients(
    const std::string& key,
    const enum WatchResponseType& type,
    std::vector<uint8_t>& oldData,
    std::vector<uint8_t>& newData) {
...
bool areEqual(const char* result, size_t result_length, const std::vector<uint8_t>& value) {
    if (result_length != value.size()) {
        return false;
    }
    return std::equal(result, result + result_length, value.begin());
}

void MemcachedStoreMasterDaemon::setHandler(int socket) {
  ...
  char* result = memcached_get(memcachedClient_, key.c_str(), key.size(), &value_length, &flags, &rc);

  if (rc == MEMCACHED_SUCCESS) {
    // Key exists in memcached, and its value is now in the 'result' pointer.
    oldData.assign(result, result + value_length);
    free(result);
    newKey = false;
  } else if (rc == MEMCACHED_NOTFOUND) {
    std::string value(newData.begin(), newData.end());
    memcached_return_t rc;
    rc = memcached_set(memcachedClient_, key.c_str(), key.size(), value.c_str(), value.size(), (time_t)0, (uint32_t)0);

    if (rc != MEMCACHED_SUCCESS) {
      // Handle error
      std::cerr << "Couldn't set key in memcached: " << memcached_strerror(memcachedClient_, rc) << std::endl;
      // Depending on your design, you may want to send a failure response to the client or throw an exception
      return; 
    }
  } else {
    // Handle other errors.
    std::cerr << "Error fetching key from memcached: " << memcached_strerror(memcachedClient_, rc) << std::endl;
  }
...
void MemcachedStoreMasterDaemon::compareSetHandler(int socket) {
  std::string key = tcputil::recvString(socket);
  ...
  if (rc == MEMCACHED_NOTFOUND) {
    if (currentValue.empty()) {
      std::string value(newValue.begin(), newValue.end());
      memcached_return_t rc;
      rc = memcached_set(memcachedClient_, key.c_str(), key.size(), value.c_str(), value.size(), (time_t)0, (uint32_t)0);
    ...
    if (areEqual(result, value_length, currentValue)) {
      std::string value(newValue.begin(), newValue.end());
      memcached_return_t rc;
      rc = memcached_set(memcachedClient_, key.c_str(), key.size(), value.c_str(), value.size(), (time_t)0, (uint32_t)0);
      ...
      // Send key update to all watching clients
      sendKeyUpdatesToClients(
          key, WatchResponseType::KEY_UPDATED, currentValue, newValue);
    }
    tcputil::sendVector<uint8_t>(socket, newValue);
    free(result);
  }
...
void MemcachedStoreMasterDaemon::addHandler(int socket) {
  std::string key = tcputil::recvString(socket);
  ...
  if (rc == MEMCACHED_SUCCESS) {
    // Key exists in memcached, and its value is now in the 'result' pointer.
    oldData.assign(result, result + value_length);
    std::string oldStrValue(result);
    int64_t oldValue = std::stoll(oldStrValue);
    addVal += oldValue;
    free(result);
    newKey = false;
  } else if (rc == MEMCACHED_NOTFOUND) {
...
void MemcachedStoreMasterDaemon::getHandler(int socket) const {
  std::string key = tcputil::recvString(socket);
  size_t value_length;
  uint32_t flags;
  memcached_return_t rc;
  ...
  std::vector<uint8_t> data(result, result + value_length);

  tcputil::sendVector<uint8_t>(socket, data);
}

void MemcachedStoreMasterDaemon::getNumKeysHandler(int socket) const {
  throw std::runtime_error("Function not yet implemented");
  // tcputil::sendValue<int64_t>(socket, tcpStore_.size());
}

void MemcachedStoreMasterDaemon::deleteHandler(int socket) {
  std::string key = tcputil::recvString(socket);
  ...
  if (rc == MEMCACHED_SUCCESS) {
    std::vector<uint8_t> oldData(result, result + value_length);
    std::vector<uint8_t> newData;

    sendKeyUpdatesToClients(key, WatchResponseType::KEY_DELETED, oldData, newData);
    memcached_return_t rc = memcached_delete(memcachedClient_, key.c_str(), key.length(), (time_t)0);

    tcputil::sendValue<int64_t>(socket, 1);
  } else {
    tcputil::sendValue<int64_t>(socket, 0);
  }
}

void MemcachedStoreMasterDaemon::checkHandler(int socket) const {
  SizeType nargs = 0;
...
void MemcachedStoreMasterDaemon::waitHandler(int socket) {
  SizeType nargs = 0;
  tcputil::recvBytes<SizeType>(socket, &nargs, 1);
  std::vector<std::string> keys(nargs);
  for (const auto i : c10::irange(nargs)) {
  ...
  if (checkKeys(keys)) {
    ...
    for (auto& key : keys) {
      memcached_return_t rc = memcached_exist(memcachedClient_, key.c_str(), key.size());
...
bool MemcachedStoreMasterDaemon::checkKeys(
  ...
  return std::all_of(keys.begin(), keys.end(), [this](const std::string& s) {
    ...
    memcached_return_t rc = memcached_exist(memcachedClient_, s.c_str(), s.size());

    // If result is not null, then the key exists
    bool keyExists = rc == MEMCACHED_SUCCESS;
...
#ifdef _WIN32
void MemcachedStoreMasterDaemon::run() {
  ...
  while (!finished) {
    ...
    // MemcachedStore's listening socket has an event and it should now be able to
...
void MemcachedStoreMasterDaemon::run() {
  ...
  const char *configStr = "--SERVER=localhost:11211";

  memcachedClient_ = memcached(configStr, strlen(configStr));

  if (!memcachedClient_) {
    throw std::runtime_error("Failed to initialize Memcached client");
  }
...
class MemcachedStoreWorkerDaemon : public BackgroundThread {
 public:
  explicit MemcachedStoreWorkerDaemon(Socket&& listenSocket);
  ~MemcachedStoreWorkerDaemon() override;
...
// MemcachedStoreListener class methods
MemcachedStoreWorkerDaemon::MemcachedStoreWorkerDaemon(Socket&& listenSocket)
    : BackgroundThread{std::move(listenSocket)} {
  daemonThread_ = std::thread{&MemcachedStoreWorkerDaemon::run, this};
}

MemcachedStoreWorkerDaemon::~MemcachedStoreWorkerDaemon() {
  dispose();
}

void MemcachedStoreWorkerDaemon::setCallback(
    std::string key,
...
void MemcachedStoreWorkerDaemon::callbackHandler(int socket) {
...
#ifdef _WIN32
void MemcachedStoreWorkerDaemon::run() {
  std::vector<struct pollfd> fds;
  tcputil::addPollfd(fds, storeListenSocket_.handle(), POLLIN);
...
class MemcachedServer {
 public:
  static std::shared_ptr<MemcachedServer> start(const MemcachedStoreOptions& opts);

  std::uint16_t port() const noexcept {
    return port_;
  }

  explicit MemcachedServer(
      std::uint16_t port,
      std::unique_ptr<MemcachedStoreMasterDaemon>&& daemon)
      : port_{port}, daemon_{std::move(daemon)} {}

 private:
  std::uint16_t port_;
  std::unique_ptr<MemcachedStoreMasterDaemon> daemon_;

  // We store weak references to all MemcachedServers for which the caller requested
  // multi-tenancy.
  static std::unordered_map<std::uint16_t, std::weak_ptr<MemcachedServer>>
      cachedServers_;

  static std::mutex cache_mutex_;
};

std::unordered_map<std::uint16_t, std::weak_ptr<MemcachedServer>>
    MemcachedServer::cachedServers_{};

std::mutex MemcachedServer::cache_mutex_{};

std::shared_ptr<MemcachedServer> MemcachedServer::start(const MemcachedStoreOptions& opts) {
  auto startCore = [&opts]() {
    Socket socket = Socket::listen(opts.port);

    std::uint16_t port = socket.port();

    auto daemon = std::make_unique<MemcachedStoreMasterDaemon>(std::move(socket));

    return std::make_shared<MemcachedServer>(port, std::move(daemon));
  };

  std::shared_ptr<MemcachedServer> server{};

  if (opts.multiTenant) {
    std::lock_guard<std::mutex> guard{cache_mutex_};

    // If the caller is okay with a multi-tenant store, first check if we
    // already have a MemcachedServer running on the specified port.
    if (opts.port > 0) {
      ...
      if (pos != cachedServers_.end()) {
        ...
        // Looks like the MemcachedStore has been disposed, make sure that we release
        // the control block.
...
class MemcachedClient {
 public:
  static std::unique_ptr<MemcachedClient> connect(
      const SocketAddress& addr,
      const MemcachedStoreOptions& opts);
  ...
  explicit MemcachedClient(Socket&& socket) : socket_{std::move(socket)} {}

 private:
  Socket socket_;
};

std::unique_ptr<MemcachedClient> MemcachedClient::connect(
    const SocketAddress& addr,
    const MemcachedStoreOptions& opts) {
  ...
  return std::make_unique<MemcachedClient>(std::move(socket));
}

void MemcachedClient::sendCommandForKey(QueryType type, const std::string& key) {
  tcputil::sendValue<QueryType>(socket_.handle(), type);
...
void MemcachedClient::sendStrings(c10::ArrayRef<std::string> value) {
...
void MemcachedClient::setTimeout(std::chrono::milliseconds value) {
...
class MemcachedCallbackClient {
 public:
  static std::unique_ptr<MemcachedCallbackClient> connect(
      const SocketAddress& addr,
      const MemcachedStoreOptions& opts);

  void setCallback(const std::string& key, WatchKeyCallback callback);

  explicit MemcachedCallbackClient(
      int rawSocket,
      std::unique_ptr<MemcachedStoreWorkerDaemon>&& daemon)
      : rawSocket_{rawSocket}, daemon_{std::move(daemon)} {}

 private:
  int rawSocket_;
  std::unique_ptr<MemcachedStoreWorkerDaemon> daemon_;
  std::mutex mutex_;
};

std::unique_ptr<MemcachedCallbackClient> MemcachedCallbackClient::connect(
    const SocketAddress& addr,
    const MemcachedStoreOptions& opts) {
  auto timeout = std::chrono::duration_cast<std::chrono::seconds>(opts.timeout);
  Socket socket = Socket::connect(
  ...
  auto daemon = std::make_unique<MemcachedStoreWorkerDaemon>(std::move(socket));

  return std::make_unique<MemcachedCallbackClient>(rawSocket, std::move(daemon));
}

void MemcachedCallbackClient::setCallback(
    const std::string& key,
    WatchKeyCallback callback) {
...
MemcachedStore::MemcachedStore(std::string host, const MemcachedStoreOptions& opts)
    : Store{opts.timeout},
      addr_{std::move(host)},
  ...
  if (opts.isServer) {
    server_ = detail::MemcachedServer::start(opts);
  ...
  client_ = detail::MemcachedClient::connect(addr_, opts);

  if (opts.waitWorkers) {
    waitForWorkers();
  }

  callbackClient_ = detail::MemcachedCallbackClient::connect(addr_, opts);
}

MemcachedStore::~MemcachedStore() = default;

void MemcachedStore::waitForWorkers() {
...
void MemcachedStore::set(const std::string& key, const std::vector<uint8_t>& data) {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  client_->sendCommandForKey(detail::QueryType::SET, keyPrefix_ + key);
  client_->sendBytes(data);
}

std::vector<uint8_t> MemcachedStore::compareSet(
    const std::string& key,
...
std::vector<uint8_t> MemcachedStore::get(const std::string& key) {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  return doGet(keyPrefix_ + key);
}

std::vector<uint8_t> MemcachedStore::doGet(const std::string& key) {
  doWait(key, timeout_);
  client_->sendCommandForKey(detail::QueryType::GET, key);
  return client_->receiveBits();
}

int64_t MemcachedStore::add(const std::string& key, int64_t value) {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  return incrementValueBy(keyPrefix_ + key, value);
}

bool MemcachedStore::deleteKey(const std::string& key) {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
void MemcachedStore::watchKey(const std::string& key, WatchKeyCallback callback) {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  callbackClient_->setCallback(keyPrefix_ + key, callback);
}

int64_t MemcachedStore::incrementValueBy(const std::string& key, int64_t delta) {
...
int64_t MemcachedStore::getNumKeys() {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  client_->sendCommand(detail::QueryType::GETNUMKEYS);
  return client_->receiveValue<std::int64_t>();
}

bool MemcachedStore::check(const std::vector<std::string>& keys) {
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
void MemcachedStore::wait(const std::vector<std::string>& keys) {
  wait(keys, timeout_);
}

void MemcachedStore::wait(
